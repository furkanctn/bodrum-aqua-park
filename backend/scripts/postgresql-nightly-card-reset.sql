-- =============================================================================
-- Gece yarısı kart sıfırlama (PostgreSQL sunucusunda — uygulama kapalı olsa da çalışır)
--
-- Her gece 00:00 Europe/Istanbul:
--   • cards.balance → 0
--   • cards.entry_gate → 0
...
BEGIN
	...
	SELECT
		...
		-c.balance,
		0,
		'Gece yarısı günlük sıfırlama — bakiye ve turnike hakları',
		NOW()
	FROM cards c
	WHERE c.balance > 0;
...
COMMENT ON FUNCTION bodrum_nightly_card_reset() IS
	'Bodrum Aqua Park: gece yarısı bakiye, entry_gate ve RFID pas sıfırlama';

-- -----------------------------------------------------------------------------
-- Zamanlama A — pg_cron (PostgreSQL eklentisi, sunucuda yüklüyse önerilir)
-- Sunucu saat dilimi Europe/Istanbul ise: 0 0 * * *
-- Sunucu UTC ise (Türkiye sabit UTC+3): 0 21 * * *
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 0 0 * * * psql -h 127.0.0.1 -U postgres -d bodrum_aqua_park -c "SELECT bodrum_nightly_card_reset();" >> /var/log/bodrum-nightly-reset.log 2>&1
